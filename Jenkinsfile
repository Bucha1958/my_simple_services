...
pipeline {
    ...
    stages {
        stage('Build JAR') {
            steps {
                echo 'Simulating Maven build (mvn clean package)...'
            }
        }

        stage('Build Docker Image') {
            steps {
                echo 'Simulating Docker image build (docker build -t bucha1958/my_java_app:latest .)...'
            }
        }

        stage('Push Docker Image') {
            steps {
                echo 'Simulating DockerHub login and image push (docker push bucha1958/my_java_app:latest)...'
            }
        }
    }
}
